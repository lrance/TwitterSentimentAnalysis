...
short_pos = open("hanzhe_GMO_DataSet/SearchAPI_hanzheData_high_pro.txt","r").read()
short_low_pos = open("hanzhe_GMO_DataSet/SearchAPI_hanzheData_low_pro.txt","r").read()
short_neg = open("hanzhe_GMO_DataSet/SearchAPI_hanzheData_high_con.txt","r").read()
short_low_neg = open("hanzhe_GMO_DataSet/SearchAPI_hanzheData_low_con.txt","r").read()
...
def evaluate_features(feature_select):
    ...
    for i in short_low_pos.split('\n'):
        low_posWords = word_tokenize(i)
        low_posWords_tag = [feature_select(low_posWords),"low_pos"]
        #post each word as "pos" in positive dataset
        low_posFeatures.append(low_posWords_tag)

        #tag word type for each word like, "NN","J", j is adjective, r is adverb, and v is verb
        pos = nltk.pos_tag(low_posWords)
        for w in pos:
            if w[1][0] in allowed_word_types:
                all_words.append(w[0].lower())
                documents.append("low_pos")
    ...
    for i in short_low_neg.split('\n'):
        low_negWords = word_tokenize(i)
        low_negWords_tag = [feature_select(low_negWords),"low_neg"]
        #post each word as "pos" in positive dataset
        low_negFeatures.append(low_negWords_tag)

        #tag word type for each word like, "NN","J", j is adjective, r is adverb, and v is verb
        pos = nltk.pos_tag(low_negWords)
        for w in pos:
            if w[1][0] in allowed_word_types:
                all_words.append(w[0].lower())
                documents.append("low_neg")   
    #get 10-Fold cross validation for Accuracy,Recall,Prediction
    num_folds = 10
    training = posFeatures + negFeatures + low_posFeatures + low_negFeatures
    cv = cross_validation.KFold(len(training),n_folds=10, shuffle=True, random_state=None)
...
def create_word_scores():
    ...
    for i in short_low_pos.split('\n'):
        low_posWords = word_tokenize(i)
        low_posWord_score.append(low_posWords)

    for i in short_low_neg.split('\n'):
        low_negWords = word_tokenize(i)
        low_negWord_score.append(low_negWords)
        
    word_scores = {}

    posWord_score = list(itertools.chain(*posWord_score))
    negWord_score = list(itertools.chain(*negWord_score))
    low_posWord_score = list(itertools.chain(*low_posWord_score))
    low_negWord_score = list(itertools.chain(*low_negWord_score))
    ...
    for word in low_posWord_score:
        word_fd[word.lower()] += 1
        cond_word_fd["low_pos"][word.lower()] += 1
        
    for word in low_negWord_score:
        word_fd[word.lower()] += 1
        cond_word_fd["low_neg"][word.lower()] += 1
        
    #finds the number of positive and negative words, as well as the total number of words
    pos_word_count = cond_word_fd["pos"].N()
    neg_word_count = cond_word_fd["neg"].N()
    low_pos_word_count = cond_word_fd["low_pos"].N()
    low_neg_word_count = cond_word_fd["low_neg"].N()
    
    total_word_count = pos_word_count + neg_word_count + low_pos_word_count + low_neg_word_count
    ...
    for word, freq in word_fd.items():
        ...
        low_pos_score = BigramAssocMeasures.chi_sq(cond_word_fd["low_pos"][word], (freq, pos_word_count), total_word_count)
        low_neg_score = BigramAssocMeasures.chi_sq(cond_word_fd["low_neg"][word], (freq, neg_word_count), total_word_count)
        word_scores[word] = pos_score + neg_score + low_pos_score + low_neg_score
